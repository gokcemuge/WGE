...
class Model(nn.Module):
    def __init__(self, name, kg, embedding_dim, batch_size, learning_rate, neg_size, gpu=True, regul=False, train_with_groundings=False, train_with_psl=False, load_model=False, save_model=False):
        ...
        # Nets
        # For each implementation model you have to define their score function and their initialization
        self.init_embedding()
    ...
    def calculate_score(self, head, tail, relation):
        ...
        if self.name == 'RotatE':
            ...
            # TODO: it is said that regularizer is not being used during the testing phase in original
            if self.regul: # if it is a positive triples returns outs from here else from above
