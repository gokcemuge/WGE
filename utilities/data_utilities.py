...
class KnowledgeGraph:
    ...
    def load_hr_map(self, test_triples, training_triples, validation_triples):
        ...
        # if self.dataset_name == 'nl27k':
        if self.dataset_name == 'dummy':
            return
        else:
            self.hr_map = {}
            # base query triples
...
# same function as the original implementation
# order is updated as [h,t,r,w] -- softlogics zaten bu order da geliyo
...
def gen_psl_samples(softlogics, n_psl_triples):
    ...
    # soft_h, soft_t, soft_r, soft_lb = samples[:,0].astype(int),samples[:,2].astype(int),samples[:,1].astype(int), samples[:,3]
    # soft_sample_batch = (soft_h, soft_t, soft_r, soft_lb)
    return samples
