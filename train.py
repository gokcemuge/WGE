...
from utilities.evaluation_gokce import *
...
def train(name, out_file, model, data_dir, dim, batch_size,
    ...
    for epoch in range(max_epoch):
        ...
        #grounded_triples = list(get_minibatches(groundings_all, 1, shuffle=False))
        ...
        ''' 
        if trainings size is 5 and batcb size is 2, 
        print("train_triple size", len(train_triple)) returns 3
        print("train_triple size other", (kg.n_training_triple // batch_size)) returns 2
        '''
        ...
        for iter_triple, grounding_batch in itertools.zip_longest(train_triple, grounded_triples):  # h t r
            ...
            if model.name == "UKGE_rect":  # TODO: not: rect no bound versiyonu zaten bu
                ...
                if model.train_with_psl:
                    ...
                    psl_scores = model.forward(psl_triples_batch)
                    # TODO: not: burada bound yok evaluationlarda var
                    psl_loss = UKGE_psl_loss(model, psl_weights, psl_scores)
            ...
            total_loss = loss
            ''' 
            # HERE multiplied by iteration batch size (then later below divided by total instance)
            pos_loss_epoches_list.append(pos_loss.item()*iter_triple_size)
            neg_loss_epoches_list.append(negative_loss.item() * iter_triple_size)
            '''
        ...
        if model.name == "transE" or model.name == 'distmult' or model.name == 'complEx':
            if (epoch + 1) % validate_epoch == 0:
                ...
                model.eval()
                with torch.no_grad():
                    # kg.train_val_test_triple_filter
                    # decision_tree_classify(model, 0.85, train_pos, test_pos, entity_list, kg.all_pos_triples, )
            ...
            if (epoch+1) % validate_epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    # decision_tree_classify(model, 0.85, train_pos, val_pos, test_neg)
                    ...
                    val_duration = time.time() - val_start_time
                    print('mse tot: {:.4f} - mae tot: {:.4f} - val time: {:.1f}'.format(mse_pos.item()+mse_neg.item(),
                                                                          mae_pos.item()+mae_neg.item(), val_duration))

                    #  e = Evaluation(model)
                    #  mndcg, endcg = e.mean_ndcg(model, hr_map200)  # this method calculates and prints ndcg
...
if __name__ == '__main__':
    ...
    negative_sample = int((args.negative_sample / 2))  # TODO: when using UKGE, you may enter x2 times
    ...
    '''
    # best parameter combination for LOGI UKGE
